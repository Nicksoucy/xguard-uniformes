        ...
        <div class="min-h-screen gradient-bg">
            ...
            <div class="max-w-6xl mx-auto p-6">
                ...
                <div id="employee-cards-list" class="grid md:grid-cols-2 gap-6">
                    ${(app.showInactive ? inactiveEmployees : employees).map(emp => renderEmployeeCard(app, emp)).join('')}
                </div>
            </div>
        </div>
    `;
...
function renderEmployeeCard(app, emp) {
...
export function renderSelectEmployee(app) {
    return `
        <div class="min-h-screen gradient-bg">
            <!-- Header -->
            <div class="glass-effect shadow-lg">
                <div class="max-w-4xl mx-auto p-4 flex justify-between items-center">
                    <h1 class="text-xl font-bold text-gray-800">
                        ${app.transactionType === 'attribution' ? 'Nouvelle attribution' : 
            ...
            <div class="max-w-4xl mx-auto p-6 animate-fade-in">
                <div class="bg-white rounded-2xl shadow-xl p-8">
                    ...
                    <div id="employee-list" class="space-y-3 max-h-96 overflow-y-auto">
                        ${app.db.data.employees
                            .filter(e => e.active)
                            .sort((a, b) => a.name.localeCompare(b.name))
                            .map(emp => renderEmployeeSelection(app, emp))
                            .join('')}
                    </div>
                </div>
            </div>
...
// Rendu d'un employé dans la sélection
function renderEmployeeSelection(app, emp) {
    const balance = app.db.getEmployeeBalance(emp.id);
    const totalItems = balance.reduce((sum, item) => sum + item.quantity, 0);
...
export function renderNewEmployee(app) {
    return `
        <div class="min-h-screen gradient-bg">
            <!-- Header -->
            <div class="glass-effect shadow-lg">
...
export function renderEmployeeDetails(app) {
    const employee = app.db.getEmployee(app.currentEmployee);
    if (!employee) {
        return renderError('Employé introuvable', 'Cet employé n\'existe pas.');
    }

    const transactions = app.db.getEmployeeTransactions(app.currentEmployee);
        ...
        <div class="min-h-screen gradient-bg">
            ...
            <div class="max-w-6xl mx-auto p-6">
                ...
                <div class="bg-white rounded-xl shadow-lg p-6">
                    ...
                    ${renderTransactionHistory(app, transactions)}
                </div>
            </div>
        </div>
    `;
...
function renderTransactionHistory(app, transactions) {
    if (transactions.length === 0) {
        return '<p class="text-gray-500 text-center py-8">Aucune transaction</p>';
    }
            ...
            ${transactions.map(t => {
                ...
                return `
                    <div class="border rounded-lg p-4">
                        ...
                        ${t.signed ? `
                            <div class="mt-2">
                                <button onclick="app.viewTransactionPDF('${t.id}')" 
...
// Event handlers pour les employés
export function attachNewEmployeeEvents(app) {
    ...
    form.addEventListener('submit', (e) => {
        ...
        const newEmp = app.db.addEmployee(employee);
        app.currentEmployee = newEmp.id;
        app.currentView = 'transaction';
        app.render();
    });
}

export function attachSelectEmployeeEvents(app) {
